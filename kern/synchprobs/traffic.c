...
static int NumIterations = 10;  // number of vehicle arrivals per thread
static int NumThreads = 5;      // number of concurrent simulation threads
...
print_perf_stats(void) {
  int i;
  int wait_msecs,mean_wait_msecs;
  ...
  /* first print average wait time for each direction of origin */
  for(i=0;i<4;i++) {
    ...
    if (wait_count[i] > 0) {
      ...
      kprintf("%d vehicles, average %d.%03d seconds waiting\n",wait_count[i], mean_wait_msecs/1000,mean_wait_msecs%1000);
    } else {
      kprintf("0 vehicles, average 0.000 seconds\n");
    }
  }
		  ...
		  char ** args)
{
  ...
  if ((nargs != 1) && (nargs != 5)) {
    kprintf("Usage: <command> [threads iterations interarrivaltime servicetime\n");
  ...
  kprintf("Threads: %d Iterations: %d Interarrival time: %d  Service time: %d\n",
          NumThreads,NumIterations,InterArrivalTime,ServiceTime);
